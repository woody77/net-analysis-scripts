"""
This generates graphs from a series of results of IPerf runs, with the data exported in JSON format

 The general usage is:

 python IPerfAnalyze.py -i "dataset1 name" "dataset1 file" [-i "dataset2 name" "dataset2 file"]
...
arg_parser.add_argument('--data', nargs='+', action=NameValuePairArgList, required=True,
                        help='<label>=<filename> pairs of labels and IPerf3 JSON files')
...
class Sample(object):
    ...
    def __init__(self, bytes_sent, seconds, retrans, cwnd, rtt):
        self.bytes = bytes_sent
        self.seconds = seconds
        self.retransmits = retrans
        self.cwnd = cwnd
        self.rtt = rtt
        self.bw = bytes_sent * 8.0 / seconds
...
def extract_stream_data(json_data, print_samples=False):
    ...
    # print ""
    # print "Intervals:"
    parsed_data = [Sample(raw_sample['bytes'], raw_sample['seconds'], raw_sample['retransmits'], raw_sample['snd_cwnd'],
                          raw_sample['rtt']) for raw_sample in
                   [interval['streams'][0] for interval in json_data['intervals']]]
    if print_samples:
        for sample in parsed_data:
            print "seconds: {:1.3f}, bytes: {:9d}, retrans: {:3d}, cwnd: {:6d}, rtt: {:6d}, result: {:3.1f}".format(
                sample.seconds, sample.bytes, sample.retransmits, sample.cwnd, sample.rtt, sample.bw)
    return parsed_data
...
for dataset in args.data:
    ...
    datasets.append((dataset_name, extract_stream_data(get_throughput_json(dataset_file)), color_map[color_id]))
...
if "pps" in args.graphs:
    ...
    build_time_graph()
    build_count_graph([(name, [s.bw / (8*1332) for s in data], color) for name, data, color in datasets], counts_of="packets/second")
    build_legend(loc=4)  # lower right, which works better for this particular one
    save_graph("pps")

"""
This is a graph of TCP retransmits, over time
"""

if "retransmits" in args.graphs:
    build_graph("Retransmits")
    build_time_graph()
    build_count_graph([(name, [s.retransmits for s in data], color) for name, data, color in datasets],
                      counts_of='retransmits')
    build_legend()
    save_graph("retransmits")

"""
This is a graph of the TCP congestion window size, over time
"""

if "cwnd" in args.graphs:
    build_graph("TCP Congestion Window")
    build_time_graph()
    yticks = range(0, 1024 * 1024 + 1, 256 * 1024)
    plt.yticks(yticks, [t / 1024 for t in yticks])
    for name, data, color in datasets:
        plt.plot([s.cwnd for s in data], label=name, color=color)
    build_legend()
    save_graph("cwnd")

if "cwnd_pkts" in args.graphs:
    build_graph("TCP Congestion Window (Packets)")
    build_time_graph()
    build_count_graph([(name, [s.cwnd / 1332 for s in data], color) for name, data, color in datasets],
                      counts_of='packets')
    build_legend()
    save_graph("cwnd_pkts")

"""
This is a graph of the RTT, as reported
"""

if "rtt" in args.graphs:
    build_graph("Round-Trip Time")
    build_time_graph()
